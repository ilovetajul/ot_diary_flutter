...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // TextEditingController গুলো track করুন — key দিয়ে rebuild হলে reset হবে
  ...
  // ─── সব controller clear করুন ───
  void _clearControllers() {
    for (final c in _controllers.values) c.dispose();
    _controllers.clear();
  }

  // ─── Firebase থেকে fresh data লোড ───
  Future<void> _loadData() async {
    ...
    _clearControllers(); // পুরনো controller মুছুন
    ...
    // Controller গুলো নতুন data দিয়ে তৈরি করুন
    _initControllers();
  }

  // ─── প্রতিটি দিনের জন্য controller তৈরি ───
  ...
  Future<void> _resetMonth() async {
    ...
    if (confirm == true) {
      await DatabaseService.resetMonth(
          _currentYear, _currentMonth + 1);
      _clearControllers();
      setState(() => _otData = {});
      _initControllers();
    ...
    return Container(
      ...
      child: Text(text,
          style: TextStyle(
              ...
              fontWeight:
                  accent ? FontWeight.w700 : FontWeight.normal)),
    );
  }
  ...
  Widget _summaryCards() {
    return Column(children: [
      Row(children: [
        Expanded(
            child: _sCard(
                'মোট OT', '${_totalHours}h', AppColors.accent)),
        const SizedBox(width: 12),
        Expanded(
            child: _sCard('OT আয়',
                '৳${_otEarning.toStringAsFixed(0)}',
                AppColors.orange)),
      ]),
              ...
              Container(
                ...
                child: Row(children: [
                  _bItem(
                      'মূল বেতন',
                      _profile.basic > 0
                          ? '৳${_profile.basic.toStringAsFixed(0)}'
                          : '—'),
                  _divider(),
                  _bItem(
                      'ভাতা',
                      _profile.allowance > 0
                          ? '৳${_profile.allowance.toStringAsFixed(0)}'
                          : '—'),
                  _divider(),
                  _bItem(
                      'OT',
