...
class SettingsScreen extends StatefulWidget {
  ...
  const SettingsScreen(
      {super.key, required this.profile, required this.onProfileUpdated});
  @override
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  // SharedPreferences থেকে reminder status লোড করুন
  Future<void> _loadReminderStatus() async {
    final status = await NotificationService.getReminderStatus();
    if (mounted) {
      setState(() {
        _reminderOn        = status['on'];
        _reminderHour      = status['hour'];
        _reminderMin       = status['minute'];
  ...
  void _editProfile() {
    ...
    final basicCtrl = TextEditingController(
        text: widget.profile.basic.toString());
    final allowCtrl = TextEditingController(
        text: widget.profile.allowance.toString());
    final rateCtrl  = TextEditingController(
        text: widget.profile.rate.toString());
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Row(children: [
            Expanded(
                child: _modalField(basicCtrl, '💰 মূল বেতন',
                    type: TextInputType.number)),
            const SizedBox(width: 8),
            Expanded(
                child: _modalField(allowCtrl, '🎁 ভাতা',
                    type: TextInputType.number)),
            const SizedBox(width: 8),
            Expanded(
                child: _modalField(rateCtrl, '⚡ OT রেট',
                    type: TextInputType.number)),
          ]),
          const SizedBox(height: 20),
          SizedBox(
            ...
            child: DecoratedBox(
              decoration: BoxDecoration(
                gradient: const LinearGradient(
                    colors: [AppColors.accent, Color(0xFF00B89C)]),
              ...
              child: ElevatedButton(
                ...
                onPressed: () async {
                  final updated = UserProfile(
                    ...
                    basic:
                        double.tryParse(basicCtrl.text) ?? 0,
                    allowance:
                        double.tryParse(allowCtrl.text) ?? 0,
  ...
  void _setReminder() async {
    ...
    if (success) {
      ...
      _showToast('❌ রিমাইন্ডার সেট করতে সমস্যা হয়েছে');
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _settingsCard([
            _tile('✏️', 'প্রোফাইল এডিট',
                'নাম, বেতন, রেট পরিবর্তন',
